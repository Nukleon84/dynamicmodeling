...
{
    public class DecompositionSolver : IAlgebraicSolver
    {
        ...
        BasicNewtonSolver newtonSubsolver = new BasicNewtonSolver(new NoLogger());
        ...
        public string GetDebugInfo()
        {
            ...
            if (IsOverconstrained)
            {
                ...
                {
                    sb.AppendLine(String.Format("{0}", eq));
                }
            }
        ...
        public void Decompose(AlgebraicSystem problem)
        {
            ...
            foreach (var subproblem in _decomposedNlps.ToArray())
            {
                ...
                subproblem.CreateIndex();
                subproblem.GenerateJacobian();
            }
        }

        void InitialSetup(AlgebraicSystem problem)
        {
            if (Subproblems != null)
                Subproblems.Clear();
        ...
        public bool Solve(AlgebraicSystem problem)
        {
            ...
            foreach (var decomposedNlp in _decomposedNlps)
            {
                ...
                //Special Handling of Singleton Systems
                ...
                if (!status)
                {
                    ...
                    {
                        LogError(String.Format("{0,20} ( {1} )", eq.Residual().ToString("G8"), eq));
                    }
